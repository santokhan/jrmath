<template>
    <div v-for="(ddItem, ddIndex) in props.nav.dropdown" :key="ddIndex" class="w-full relative nested">
        ...
        <div class="md:absolute left-full top-0 bg-white hidden w-full md:w-56 md:shadow-lg z-[15] md:py-2 dropdown">
            <RouterLink @click="handleRoute" v-for="(link, linkIndex) in ddItem.links" :key="linkIndex" :to="link.to" :class="[
...
function handleRoute() {
    // ✅ Only call on small screen not in ❗large screen
    if (typeof props.hideDropdown === 'function') {
        props.hideDropdown()
    }
}
</script>
