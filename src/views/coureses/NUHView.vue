            ...
            <div class="w-full md:w-1/3">
                <div class="space-y-2 max-w-md bg-white rounded-lg border p-3 mt-8">
                    ...
                    <RouterLink type="button" v-for="(item, index) in videoData" :key="index"
                        @click="() => { handleVideoPlay(index) }" class="block w-full hover:bg-gray-100 rounded-lg bg-gray-50"
                        :class="vdoToPlay._id === item._id ? 'bg-orange-500 text-white hover:text-gray-700' : ''" :to="`/courses/nuh/${item._id}`">
